...
func init(player_ref: CharacterBody2D, hud_layer: CanvasLayer, map_texture: Texture2D = null) -> void:
	_player = player_ref
	var mp := hud_layer.get_node("MapPanel")
	var tr2: TextureRect = mp.get_node("MapTexture")
	if map_texture:
		tr2.texture = map_texture
	else:
		tr2.texture = AssetLoader.texture("map_full", 512, Color(0.1, 0.1, 0.15))
	var cw := 80.0
	var cols := int(ceil(5760.0 / cw))
	var rows := int(ceil(4320.0 / cw))
...
func update() -> void:
	...
	var rx := 800.0 / 5760.0
	var ry := 400.0 / 4320.0
	mk.offset_left = -400 + int(_player.global_position.x / 5760.0 * 800.0) - 10
	mk.offset_top = 30 + int(_player.global_position.y / 4320.0 * 400.0) - 12
	...
	for x in cols:
		for y in rows:
			...
			if explored:
				...
				img.set_pixel(x, y, Color(0.02, 0.02, 0.05, 0.55))
...
func is_explored(world_pos: Vector2) -> bool:
	var cw := 80.0
	var cx := int(world_pos.x / cw)
	var cy := int(world_pos.y / cw)
	if cx < 0 or cx >= _fog_grid.size() or cy < 0 or cy >= _fog_grid[0].size():
		return false
	return _fog_grid[cx][cy]
