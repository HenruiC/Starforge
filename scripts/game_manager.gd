...
func _random_spawn_position() -> Vector2:
	var cam_pos := Vector2(1600, 1200)
	if player: cam_pos = player.global_position
	var vw := 1280.0 / 1.2; var vh := 720.0 / 1.2
	var side := randi() % 4
	match side:
		0: return Vector2(randf_range(cam_pos.x - vw / 2, cam_pos.x + vw / 2), cam_pos.y - vh / 2 - spawn_margin)
		1: return Vector2(randf_range(cam_pos.x - vw / 2, cam_pos.x + vw / 2), cam_pos.y + vh / 2 + spawn_margin)
		2: return Vector2(cam_pos.x - vw / 2 - spawn_margin, randf_range(cam_pos.y - vh / 2, cam_pos.y + vh / 2))
		_: return Vector2(cam_pos.x + vw / 2 + spawn_margin, randf_range(cam_pos.y - vh / 2, cam_pos.y + vh / 2))
